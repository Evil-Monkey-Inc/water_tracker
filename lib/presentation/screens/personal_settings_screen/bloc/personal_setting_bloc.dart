...
class PersonalSettingBloc
    extends Bloc<PersonalSettingEvent, PersonalSettingState> {
  PersonalSettingBloc(this.repository) : super(InitialSettingScreenState()) {
    on<SkipPersonalSettingScreenEvent>(
    ...
    on<SaveGeneralSettingEvent>((event, emit) async {
      try {
        ...
        emit(
          isSuccess
              ? SuccessfullyPersonalSettingState()
              : ErrorPersonalSettingState(Exception()),
        );
      } catch (e) {
